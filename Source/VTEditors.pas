...
uses
  ...
  StdCtrls, VirtualTrees, Buttons, ExtCtrls, ComCtrls, Spin, Types;
...
type
  ...
  protected
    ...
    procedure FreeEditControl; virtual; // WL, 13.02.2004
...
begin
  ...
  begin
    ...
    FNode := ANode;
    FColumn := AColumn;
    FreeEditControl; // FreeAndNil(FEdit); // WL, 13.02.2004
...
procedure TCustomEditLink.FreeEditControl; // WL, 13.02.2004
begin
  FreeAndNil(FEdit);
end;
...
var
  Sep: Char;
begin
  Sep := SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator;
  try
    if FDateSeparator <> #0 then SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator := FDateSeparator;
      if FDateFormat <> '' then
    ...
    SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator := Sep;
  end;
end;
...
begin
  if Value <> '' then
  try
    DF := SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}ShortDateFormat;
    Sep := SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator;
    try
      if FDateFormat <> '' then SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}ShortDateFormat := FDateFormat;
      if FDateSeparator <> #0 then SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator := FDateSeparator;
      case FKind of
      ...
      SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}ShortDateFormat := DF;
      SysUtils.{$ifdef COMPILER_19_UP}FormatSettings.{$endif}DateSeparator := Sep;
    end;
  except
  end;
